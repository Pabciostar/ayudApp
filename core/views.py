...
@login_required
@csrf_exempt  # solo si estás teniendo problemas con CSRF, sino elimínalo
def agendar_test_view(request):
    if request.method == 'POST':
        usuario = get_object_or_404(Usuario, correo=request.user.email)
        ayudante = get_object_or_404(Ayudante, pk=request.POST.get('id_ayudante'))
        materia = get_object_or_404(Materia, pk=request.POST.get('id_materia'))

        fecha = request.POST.get('fecha')
        hora_inicio = request.POST.get('hora_inicio')
        duracion = request.POST.get('duracion_min')

        if not (fecha and hora_inicio and duracion):
            return render(request, 'error.html', {'mensaje': 'Faltan datos para agendar.'})

        fecha_obj = datetime.strptime(fecha, "%Y-%m-%d").date()
        hora_obj = datetime.strptime(hora_inicio, "%H:%M").time()

        # 🔹 Crear transacción dummy
        transaccion = Transaccion.objects.create(
            id_transaccion=int(datetime.now().strftime('%y%m%d%H%M%S')),
            voucher='TEST',
            id_payment=f'TEST-{datetime.now().strftime("%Y%m%d%H%M%S")}',
            estado='no_pagada',
            monto=0,
            moneda='USD',
            id_usuario=usuario
        ...
        # 🔹 Crear clase agendada con transacción y link_meet temporal vacío
        clase = ClaseAgendada.objects.create(
            id_clase=int(datetime.now().strftime('%y%m%d%H%M%S')),
            fecha=fecha_obj,
            hora=hora_obj,
            duracion_min=int(duracion),
            valor=Decimal(ayudante.valor),
            materia_id_materia=materia,
            usuario_id_usuario=usuario,
            id_ayudante=ayudante,
            transaccion_id_transaccion=transaccion,
            estado='confirmada',
            link_meet=''  # obligatorio según tu modelo
        )

        # 🔹 Crear evento en Google Calendar y actualizar link_meet
        enlace_meet = crear_evento_google(clase)
        if enlace_meet:
            clase.link_meet = enlace_meet
            clase.save()

        # 🔹 Crear notificaciones para estudiante y ayudante
        crear_notificacion(
            "Clase agendada (TEST)",
            "Admin",
            usuario.id_usuario,
            f"Clase TEST agendada para {fecha_obj} sobre {materia}.",
            clase.id_clase
        )
        crear_notificacion(
            "Clase agendada (TEST)",
            "Admin",
            ayudante.id_ayudante.id_usuario,
            f"Clase TEST agendada para {fecha_obj} sobre {materia}.",
            clase.id_clase
        )
    ...
    # Si no es POST, opcionalmente retorna error o formulario de test
    return render(request, 'error.html', {'mensaje': 'Método no permitido.'})
